...
public class AttributeBowSpeed {
    ...
    public static void onLivingUpdate(@Nonnull LivingEvent.LivingTickEvent evt) {
        ...
        if (!entityLivingBase.level().isClientSide() && entityLivingBase.isUsingItem()) {
            ...
            if (!itemStack.isEmpty() && (itemStack.getItem() instanceof BowItem || itemStack.getItem() instanceof CrossbowItem)) {
                float attributeValue = (float) AttributesUtil.getAttributeValue(entityLivingBase, ModAttributes.BOW_SPEED.get());
                ...
                if (speed > 1) {
                    ...
                    LogUtil.debugAttribute("弓速加成", entityLivingBase.getName().getString(), attributeValue, configValue, speed);
                }
            }
        }
    }
            ...
            if (!itemStack.isEmpty() && (itemStack.getItem() instanceof BowItem || itemStack.getItem() instanceof CrossbowItem)) {
                ...
                if (speed < 1) {
                    if (RandomUtil.percentageChance((1 - speed) * 100)) {
                        ...
                        LogUtil.debugAttribute("弓速减缓", entityLivingBase.getName().getString(), attributeValue, configValue, speed);
                    }
                }
            }
        }
